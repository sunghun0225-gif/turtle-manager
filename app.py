...
def load_data():
    if os.path.exists(DB_FILE):
        ...
        for _, row in df.iterrows():
            # [FIX 2] History가 str인지 list인지 안전하게 처리
...
def analyze_ticker(ticker):
    try:
        ...
        # [FIX 1] TR 계산: prev_close를 미리 컬럼으로 만들어 정확하고 빠르게 계산
...
def get_sec_filings(ticker: str):
    ...
    try:
        ...
        res = requests.get(url, headers={"User-Agent": "Turtle/1.0"}).json()
...
# 4. 메인 UI 및 사이드바
# ==========================================
st.set_page_config(page_title="Turtle Pro V7.36", layout="centered", page_icon="🐢")
...
st.title("🐢 Turtle System Pro V7.36")
...
with tabs[3]:
    ...
    for tkr, pos in list(st.session_state.positions.items()):
        # [FIX 3] Units를 항상 History 길이와 동기화
        ...
        # [FIX 4] avg_e=0 / total_s=0 guard → profit 계산 안전하게
        if total_s > 0:
        ...
        with st.container(border=True):
            ...
            if "눌림목" in st_n:
                ...
                lvls += [
                    {'val': tp, 'name': '6%익절', 'col': 'blue'},
                    {'val': sl, 'name': '4%손절', 'col': 'red'}
                ]
                if profit >= 0.06:
                    st.success("💰 수익실현 시점")
                elif profit <= -0.04:
                ...
                lvls += [
                    {'val': tp, 'name': '5%익절', 'col': 'blue'},
                    {'val': sl, 'name': '5%손절', 'col': 'red'}
                ]
                if profit >= 0.05:
                    st.success("💰 수익실현 시점")
                elif profit <= -0.05:
                ...
                if lt['Close'] < stop or lt['Close'] < don:
                    st.error("🛑 추세 이탈 경고")
            ...
            for lv in lvls:
                rules.append(
                    alt.Chart(pd.DataFrame({'y': [lv['val']]}))
                    .mark_rule(strokeDash=[5, 5], color=lv['col'])
                    .encode(y='y:Q')
                )
                rules.append(
                    ...
                    .mark_text(align='left', dx=5, color=lv['col'])
                    .encode(x='Date:T', y='y:Q', text='t:N')
                )
            st.altair_chart(
                alt.layer(line, *rules).properties(height=300),
    ...
    if st.session_state.positions:
        csv_data = pd.DataFrame([
            {'Ticker': k, **{kk: vv for kk, vv in v.items() if kk != 'History'}, 'History': json.dumps(v['History'])}
            for k, v in st.session_state.positions.items()
        ]).to_csv(index=False).encode('utf-8-sig')
        st.download_button(
            "💾 전체 통합 데이터 백업",
            csv_data,
            f"Turtle_Final_Backup_{datetime.now().strftime('%y%m%d')}.csv",
            "text/csv",
            use_container_width=True
        )
...
# ==========================================
with tabs[4]:
    t_in = st.text_input("상세 종목 분석 (티커)").upper()
    if t_in and st.button("분석 시작"):
        d = analyze_ticker(t_in)
        if d is not None:
            st.write(f"### {t_in} : ${d['Close'].iloc[-1]:.2f} (RSI: {d['RSI'].iloc[-1]:.1f})")
            fils = get_sec_filings(t_in)
            for f in fils:
                st.markdown(f"- [{f['form']}]({f['url']}) ({f['date']})")

with tabs[5]:
    st.subheader("🌍 글로벌 마켓 뉴스 (KST 기준)")
    f_p = feedparser.parse("https://news.google.com/rss/search?q=stock+market&hl=en-US&gl=US&ceid=US:en")
    for e in f_p.entries[:10]:
        dt = datetime(*e.published_parsed[:6]) + timedelta(hours=9)
        st.markdown(f"📍 [{e.title}]({e.link}) `[{dt.strftime('%m-%d %H:%M')}]`")
